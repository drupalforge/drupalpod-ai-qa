...
/**
 * Collects a QA AI API key for the configured provider.
 */
final class AiQaApiKeyForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state): array {
    ...
    $form['intro'] = [
      '#type' => 'item',
      '#title' => $this->t('@provider key required', [
        '@provider' => $provider_label,
      ]),
      '#markup' => '<p>' . $this->t(
        'This QA environment is configured for @provider. Enter an API key ' .
        'to continue.',
        ['@provider' => $provider_label],
      ) . '</p>',
    ];
    ...
    $form['warning'] = [
      ...
      'message' => [
        '#markup' => '<p><strong>' . $this->t('Warning:') . '</strong> ' .
          $this->t(
            'This is a QA environment. Your QA AI API key is stored in the ' .
            'site using the Key module with encryption, may still be ' .
            'accessible to privileged users or administrators, and will be ' .
            'cleared after @hours hours.',
            ['@hours' => $this->providerManager->getExpiryHours()],
          ) . '</p>',
      ],
    ];
    ...
    if ($expiry !== NULL) {
      $form['status'] = [
        '#type' => 'item',
        '#markup' => '<p>' . $this->t(
          'The current QA AI API key expires at @time.',
    ...
    $form['api_key'] = [
      ...
      '#maxlength' => 512,
      '#description' => $this->t(
        'Paste a QA AI API key for this QA session.',
      ),
    ];
    ...
    $form['actions']['submit'] = [
      '#type' => 'submit',
      '#value' => $this->t('Save QA AI API key'),
      '#button_type' => 'primary',
    ];
  ...
  public function validateForm(array &$form, FormStateInterface $form_state): void {
    ...
    if (!preg_match('/^[a-zA-Z0-9_\-+\/=.]+$/', $api_key)) {
      $form_state->setErrorByName(
        'api_key',
        $this->t(
          'API key contains invalid characters. Only letters, numbers, and ' .
          'common API key characters are allowed.',
        ),
      );
      return;
    }
    ...
    if (strlen($api_key) < 20) {
      $form_state->setErrorByName(
        'api_key',
        $this->t(
          'API key appears too short. Most API keys are at least 20 ' .
          'characters.',
        ),
      );
      return;
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    $this->providerManager->storeTemporaryKey(
      trim((string) $form_state->getValue('api_key')),
    );
    $this->messenger()->addStatus($this->t(
      'QA AI API key saved. It will expire in @hours hours.',
      ['@hours' => $this->providerManager->getExpiryHours()],
    ));
